...
void FraxWatchy::init(String datetime)
{
...
void FraxWatchy::_rtcConfig(String datetime)
{
    if(datetime != NULL)
    {
...
void FraxWatchy::handleButtonPress()
{
    ...
    else if (wakeupBit & BACK_BTN_MASK)
    {
        ...
        {
            menu.startMenu(); //exit to menu if already in app
        }
        else if (guiState == FW_UPDATE_STATE)
        {
            menu.startMenu(); //exit to menu if already in app
...
void FraxWatchy::vibMotor(uint8_t intervalMs, uint8_t length)
{
...
void FraxWatchy::showWatchFace(bool partialRefresh)
{
    display.init(0, false); //_initial_refresh to false to prevent full update on init
    display.setFullWindow();
    drawWatchFace();
...
void FraxWatchy::handleWatchFaceButton(uint8_t button)
{
    if (button == 0)
    {
        vibMotor(50, 5);
...
float FraxWatchy::getBatteryVoltage()
{
    return analogRead(ADC_PIN) / 4096.0 * 7.23;
}

uint16_t FraxWatchy::_readRegister(uint8_t address, uint8_t reg, uint8_t *data, uint16_t len)
{
...
uint16_t FraxWatchy::_writeRegister(uint8_t address, uint8_t reg, uint8_t *data, uint16_t len)
{
    Wire.beginTransmission(address);
    Wire.write(reg);
    Wire.write(data, len);
...
void FraxWatchy::_configModeCallback(WiFiManager *myWiFiManager)
{
    display.init(0, false); //_initial_refresh to false to prevent full update on init
    display.setFullWindow();
    display.fillScreen(GxEPD_BLACK);
